  ...
  <q-dialog v-model="_dialog">
    <q-card style="width: 750px; max-width: 100vw;">
      <q-card-section class="row items-center q-pa-sm q-mb-sm">
        <div class="q-ml-sm-sm">پاسخ به تیکت &nbsp; '{{ ticket.title }}'</div>
        <q-space/>
      ...
      <q-card-section class="items-center">
        ...
        bold: {label: 'Bold', icon: null, tip: 'My bold tooltip'}
          ...
          <q-card-actions class="bg-white text-teal">
            ...
            <q-btn flat :label="type === 'edit' ?  'ویرایش پاسخ': 'ثبت پاسخ'" type="submit"/>
          </q-card-actions>
...
<script>
export default {
  props: ['showDialog', 'form', 'type', 'ticket', 'question', 'answer'],
  ...
  methods: {
    onSubmit () {
      ...
      if (this.text === '<br>' || this.text === '') {
        this.showNotif('لطفا متن پاسخ را وارد کنید')
        return
      } else {
    ...
    CreateQuestion (app, formData) {
      this.$axios.post(process.env.api + 'api/panel/tickets/answers/', formData)
        .then(function () {
          app.showNotif('پاسخ مورد نظر با موفقیت ثبت شد', 'mdi-checkbox-marked-circle-outline ', 'green')
          app._dialog = false
        })
        .finally(function () {
          app.$q.loading.hide()
    ...
    UpdateQuestion (app, formData) {
      this.$axios.put(process.env.api + 'api/panel/tickets/answers/' + app.answer.id + '/', formData)
        .then(function () {
