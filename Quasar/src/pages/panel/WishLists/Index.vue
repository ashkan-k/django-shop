  ...
  <div class="q-pa-md">
    <div class="q-gutter-y-md">
      <q-card>
        ...
        >
          <q-tab class="text-primary" name="wishlists_list" icon="mdi-format-list-text" label="علاقه مندی ها"/>
        </q-tabs>
        <q-separator/>
              ...
              >
                <template v-slot:body="props">
                  <q-tr :props="props">
                    ...
                    <q-td key="setting">
                      <q-btn
                        @click="DeleteWishList(props.row.product.title  , props.row.id)"
...
export default {
  ...
  methods: {
    ...
    DeleteWishList (name, id) {
      ...
      this.$q.dialog({
        title: 'هشدار',
        message: 'آیا میخواهید محصول ' + ' ( ' + name + ' ) ' + 'از علاقه مندی های شما حذف شود ؟ ',
        ok: {
          push: true,
          label: 'تایید'
        },
        ...
        app.$axios.delete(process.env.api + 'api/panel/wishlists/' + id)
          .then(function () {
            app.showNotif('محصول مورد نظر از علاقه مندی ها با موفقیت حذف شد', 'mdi-checkbox-marked-circle-outline', 'green')
            app.GetAllWishLists()
          })
          .catch(function () {
    ...
    GetAllWishLists () {
      ...
      this.$axios.get(process.env.api + 'api/panel/wishlists/?page=' + app.paginationPage + '&search=' + app.search)
