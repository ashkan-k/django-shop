  ...
  <div class="q-pa-md">
    <div class="q-gutter-y-md">
      <q-card>
        ...
        >
          <q-tab class="text-primary" name="users_list" icon="mdi-format-list-text" label="کاربران"/>
        </q-tabs>
        <q-separator/>
                ...
                <template v-slot:body="props">
                  <q-tr :props="props">
                    ...
                    <q-td key="setting">
                      ...
                      |
                       <q-btn @click="BlockUser(props.row.username , props.row.id)" style="color: red" rounded
                             icon="mdi-trash-can-outline" label="مسدود کردن" size="md" flat dense>
                        <q-tooltip>مسدود کردن</q-tooltip>
                      </q-btn>
                    </q-td>
                  </q-tr>
                </template>
  ...
  methods: {
    ...
    DeleteUser (name, id) {
      ...
      this.$q.dialog({
        ...
        app.$axios.delete(process.env.api + 'api/admin/users/' + id)
    ...
    EditUser (id) {
      ...
      this.$q.loading.show()
      this.$axios.get(process.env.api + 'api/admin/users/' + id + '/')
        .then(function (response) {
    ...
    BlockUser (name, id) {
      ...
      this.$q.dialog({
        title: 'هشدار',
        message: 'آیا میخواهید کاربر ' + ' ( ' + name + ' ) ' + 'مسدود شود ؟ ',
        ok: {
          push: true,
          label: 'تایید'
        },
        ...
        app.$axios.post(process.env.api + 'api/admin/users/change/block_status/', formData)
          .then(function () {
            app.showNotif('کاربر مورد نظر با موفقیت مسدود شد', 'mdi-checkbox-marked-circle-outline', 'green')
            app.GetAllUsers()
          })
          .catch(function () {
            app.showNotif('مشکلی به وجود آمده است')
    ...
    GetAllUsers () {
      ...
      this.$axios.get(process.env.api + 'api/admin/users/?page=' + app.paginationPage + '&search=' + app.search)
        .then(function (response) {
